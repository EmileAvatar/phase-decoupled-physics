...
| 89.8 | sigma_DM/m = 4 pi G^2 m_DM/v^4 = 5.2e-49 m^2/kg << Bullet Cluster bound  [DM self-interaction; CORRECTED Part 118 — original G/c^4 form was dimensionally inconsistent] | [DERIVED, corrected 118] |
...
### Part 91 additions (Koide theta_0 -- A4 FCC):

| Eq.   | Expression | Status | Source |
|-------|------------|--------|--------|
| 91.1  | sqrt(m_i) = mu*(1+sqrt(2)*cos(theta_0+2*pi*i/3)) | [DERIVED, Part 53] | Brannen |
| 91.2  | m_e=0.511, m_mu=105.66, m_tau=1776.86 MeV; m_u=2.16, m_c=1273, m_t=172690 MeV | [EMPIRICAL] | PDG 2023 |
| 91.3  | theta_0(lep)=2/9, theta_0(up)~2/27 (+0.6%), theta_0(dn)~1/9 (+0.9%) | [SPECULATIVE] | PDTP Original |
| 91.4  | theta_0(sector) ~ 2/3^n pattern (n=2 lep, n=3 up) | [SPECULATIVE] | PDTP Original |
| 91.5  | Q_lep=2/3 (Z3-neutral); Q_quark != 2/3 (Z3-charged) | [DERIVED] | PDTP Original |
| 91.6  | Q_nu = 2/3, delta_nu = sqrt(2) [ASSUMED for neutrinos] | [ASSUMED] | PDTP Original |
| 91.7  | Dm2_21=7.53e-5 eV^2, Dm2_31=2.453e-3 eV^2 (NO) | [EMPIRICAL] | NuFIT 5.3 2023 |
| 91.8  | theta_nu = 0.4834 rad (NO, from ratio R=32.57) | [DERIVED] | PDTP Original |
| 91.9  | m_nu1=0.37 meV, m_nu2=8.69 meV, m_nu3=49.5 meV, Sum=58.6 meV (NO) | [DERIVED] | PDTP Original |
| 91.10 | Q_nu = 0.52 (signed Brannen; f_1 < 0 for lightest nu) | [DERIVED] | PDTP Original |
| 91.11 | Sum(m_nu) = 58.6 meV [FALSIFIABLE: CMB-S4/Euclid ~2030] | [FALSIFIABLE] | PDTP Original |
| 91.12 | SU(5) Z5 angles all >20% off theta_0 [NEGATIVE] | [NEGATIVE] | PDTP Original |
| 91.13 | Best PDTP scan angle: C2_fund/(2*pi)=4.5% off [NEGATIVE] | [NEGATIVE] | PDTP Original |

**Conclusion (Part 91):** A4 CONFIRMED FREE PARAMETER (12/12 PASS). theta_0 = 2/9 not
generated by SU(5) center, Casimir angles, or any PDTP-derived angle. Cross-sector pattern
theta_0 ~ 2/3^n is suggestive but unconfirmed (quark mass uncertainty). New PDTP Original
result: neutrino masses predicted as Sum=58.6 meV (NO) from Koide + oscillation data,
testable by CMB-S4/Euclid. Finding: lightest neutrino in signed Brannen regime (f_1 < 0),
giving Q_nu = 0.52, distinct from charged lepton Q = 2/3.
...
erratum block in `docs/research/condensate_layer_optics.md`. Sudoku 7/7 PASS.
...
### Part 117 additions (T44: Positive phi_-^4 Quartic Origin):

**Source:** Part 117 (Phase 85, 2026-06-11), `simulations/solver/phi_minus_quartic.py`;
`docs/research/phi_minus_quartic.md`. Sudoku 10/10 PASS. Closes Part 88 "missing phi_-^4" diagnosis.

| Eq # | Equation / result | Tag | Notes |
|------|-------------------|-----|-------|
| 117.5 | V(0,chi) = -2g cos(chi): quartic = -g/12 | [VERIFIED] | tree channel wrong sign — confirms Part 88 Eq 88.13 exactly |
| 117.8 | V_zp = (1/2)sqrt(2g cos chi): quartic = -sqrt(2)sqrt(g)/192 | [DERIVED, NEGATIVE] | zero-point channel also wrong sign (single-mode approx) |
| 117.13 | V_min = V_0 - J^2/(2M) with J = 2g sin(beta) sin(phi_-) | [VERIFIED, textbook Gaussian matching] | integrating out phi_+ fluctuation at partial lock psi - phi_+ = pi/2 - beta |
| 117.14 | V_ind = -(2g^2 sin^2(beta)/kbar^2) sin^2(phi_-) | [PDTP Original, DERIVED] | gradient-dominated regime kbar^2 >> M_pot |
| 117.16 | lambda_4 = +2g^2 sin^2(beta)/(3 kbar^2) > 0; = (g/3) sin^2(beta) at kbar^2 = 2g | [PDTP Original, DERIVED] | REQUIRED positive EDE quartic; Part 88 needs ~g (factor 3-6); sign SymPy-verified |
| 117.18 | J = V_ind = lambda_4 = 0 identically at beta = 0 | [VERIFIED] | self-switching: EDE transient by construction; w = -1 today automatic; Parts 61/63/87 untouched |

Key results: the EDE-enabling positive quartic EXISTS in PDTP — generated by phi_+
back-reaction while phase locking is incomplete, vanishing automatically at full lock.
Shape and order of magnitude derived; amplitude needs the locking history beta(z) [OPEN].

---

### Part 116 additions (T43: DM Winding Number Selection):

**Source:** Part 116 (Phase 84, 2026-06-11), `simulations/solver/dm_winding_selection.py`;
`docs/research/dm_winding_selection.md`. Sudoku 12/12 PASS. Closes "DM mass free parameter" (Part 96 D3).

| Eq # | Equation / result | Tag | Notes |
|------|-------------------|-----|-------|
| 116.5 | E/L = pi rho_s (hbar/m_cond)^2 n^2 ln(R/r_c) | [DERIVED, SymPy residual 0] | vortex line energy; n^2 scaling is output of integration |
| 116.8 | Delta E = A[n^2 ln(Lambda/n) - n ln(Lambda)] > 0 for n >= 2 | [DERIVED] | splitting instability WITH PDTP core condition r_c = n*lambda_cond; only n = 1 stable |
| 116.9 | KZ Monte Carlo: P(\|n\|>=2)/P(\|n\|=1) = 0.038 | [DERIVED, numerical, 2M loops] | defect formation makes (almost) only \|n\| = 1 |
| 116.10 | n = 1 -> m_DM = m_P = 1.22e19 GeV ("Planck vortex relic") | [PDTP Original, DERIVED] | DM mass no longer free, given bare-vortex premise; r_core = l_P |
| 116.15 | Y_req = n_DM/s = 3.6e-29 | [DERIVED] | required relic abundance per entropy quantum |
| 116.16 | KZ-before-inflation: Y/Y_req = 1.9e-50 | [DERIVED, NEGATIVE] | monopole-problem logic; abundance needs post-inflation production (PIDM; defect channel at preheating OPEN) |

Key results: winding selected (n = 1) by stability + Kibble-Zurek; all observational
tests pass with huge margins; abundance NOT from KZ — kill test: CMB tensor modes
(LiteBIRD/CMB-S4 null result kills Planck-vortex DM).
