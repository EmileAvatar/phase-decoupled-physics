...
def run_cosmo_constant_v2_phase(rw, engine):
    ...
    rw.print("")
    rw.print("  CORE RESULTS [DERIVED]:")
    rw.print("    - Two dispersion branches: Jeans (unstable) + breathing (gapped)")
    rw.print("    - Two-phase T_mu_nu: T_00 = T_kin - V (SymPy verified)")
    rw.print("    - U(1) shift preserves psi-phi_+ but NOT phi_-")
    rw.print("    - phi_- = Goldstone-like at equilibrium (flat direction)")
    rw.print("    - Dispersion: omega^2 = c^2*k^2 +/- 2*sqrt(2)*g")
    rw.print("    - Jeans wavelength = 2*pi*l_P (Planck scale)")
    rw.print("    - Mode splitting kappa = omega_gap^2 [derived, NOT free]")
    rw.print("    - rho ~ c^2/(4*pi*G*L_H^2) = (2/3)*rho_crit")
    rw.print("    *** Omega = 2/3 EXACTLY (vs Omega_Lambda=0.685, 2.6% off)")
    rw.print("")
    rw.print("  STATUS TAGS (updated per ChatGPT review):")
    rw.print("    [DERIVED]  Two-phase field structure, dispersion, mode splitting")
    rw.print("    [CONSISTENCY RELATION]  Omega = 2/3 (uses G and H_0 as inputs)")
    rw.print("    [REJECTED] 'beat frequency = dark energy' (121 orders off at Hubble)")
    rw.print("    [ANSATZ]   'one quantum per Hubble mode' (not derived from Lagrangian)")
    rw.print("    [CANDIDATE] phi_- as dark energy field (correct DOF, unproven mechanism)")
    rw.print("")
    rw.print("  NEGATIVE RESULTS:")
    rw.print("    - phi_- naive ZPE = rho_Planck (CC problem NOT solved by two-phase alone)")
    rw.print("    - Beat frequency at Hubble scale is 121 orders off H_0 [REJECTED]")
    rw.print("    - 'One quantum per Hubble mode' is an ANSATZ, not derived")
    rw.print("    - Interface energy 27 decades too large [REJECTED]")
    rw.print("")
    rw.print("  WHAT THE REAL PHYSICS IS (after ChatGPT review):")
    rw.print("    Two-phase Lagrangian -> two dispersion branches -> mode splitting")
    rw.print("    -> low-energy branch (phi_-) evaluated at cosmological scale")
    rw.print("    -> density scaling rho ~ 1/(G*L_H^2)")
    rw.print("    -> Omega ~ O(1), specifically 2/3")
    rw.print("    This is ALREADY better than standard QFT (which gives 10^122).")
    rw.print("    The 2/3 is a consistency relation, not yet a prediction.")
    rw.print("")
    rw.print("  OPEN QUESTION (critical next step):")
    rw.print("    Derive phi_- mode occupation from first principles.")
    rw.print("    WHY does phi_- populate modes at Hubble scale?")
    rw.print("    WHY is the energy density rho ~ 1/(G*L_H^2)?")
    rw.print("    The 'one quantum per Hubble mode' ansatz MUST be derived or abandoned.")
    rw.print("")
    rw.print("  CONCLUSION:")
    rw.print("    1. phi_- is the correct DOF for dark energy [CANDIDATE]")
    rw.print("    2. Omega = 2/3 is a CONSISTENCY RELATION (not prediction)")
    rw.print("    3. The two-phase structure gives O(1) instead of 10^122 [progress]")
    rw.print("    4. Beat frequency interpretation is REJECTED [121 orders off]")
    rw.print("    5. Mode splitting reframe is correct but strong-coupling regime")
    rw.print("    6. Path forward: derive phi_- energy density from field theory")
    rw.print("       without the 'one quantum per mode' assumption")
    rw.print("")
