...
**Status:** PARTIAL — gap characterized, not closed. (T12/Part 129, 2026-08-09:
checked whether n_PDTP narrows the gap — NEGATIVE, see Section 10. Gap unchanged.)
**Sudoku:** 10/10 PASS (+ 12/12, T12 addition — Section 10)
...
## 10. T12 — Does n_PDTP Modify the N_eff Factor? (Part 129, 2026-08-09)

**Status:** DONE. NEGATIVE result — N_eff gap unaffected.
**PDTP Original:** n_PDTP cannot enter the Sakharov cutoff calculation because
the calculation is conducted entirely at M=0, where n_PDTP is identically 1
[DERIVED, NEGATIVE-BUT-INFORMATIVE].
**Date:** 2026-08-09
**Script:** `simulations/solver/t12_neff_refractive_index.py`
**Log:** `simulations/solver/outputs/t12_neff_refractive_index_20260809_200131.txt`
**Sudoku:** 12/12 PASS
**Source:** TODO_04.md T12 — "Does n_PDTP = 1/alpha enter the heat kernel
expansion? Could this modify the 6π factor?"

### 10.1 Plain English Summary

PDTP has a refractive index, n_PDTP = 1/α, that describes how matter/light
waves slow down near a gravitating mass (Part 98). It seemed natural to ask:
does this refractive index sneak into the Sakharov calculation that sets N_eff
(the "how many quantum fields are buzzing in the vacuum" count needed to
reproduce Newton's constant, Part 83)?

**No — and the reason is clean.** n_PDTP is defined by α = √(1 − 2GM/rc²): it
only differs from 1 when there is an actual mass M nearby. But the Sakharov
calculation that fixes N_eff is done in flat, empty vacuum — that is the whole
point of "induced gravity": G itself is generated by vacuum zero-point
fluctuations with no gravitating body present yet. Set M = 0 in the refractive
index formula and it becomes exactly 1, everywhere. There is nothing for a
refractive-index correction to modify — the two calculations simply never
overlap. The N_eff gap characterized in Part 83 (needs ~19 effective fields,
have between 8 and 34 depending on what's counted) is exactly as open as it
was before this check.

### 10.2 The Argument [DERIVED]

**Step 1** [Part 95, Eq 95.5/95.7]: the C1 condensate's dispersion relations
— ω² = c²k² (photon/phonon) and ω² = (m_cond·c²/ℏ)² + c²k² (vortex/matter) —
contain no α (equivalently, no n_PDTP) term. **[VERIFIED, SymPy: α is not a
free symbol in either expression.]** This is the dispersion the Sakharov
vacuum sum is actually built from.

**Step 2** [Part 98, Eq 98.2]: α(r) = √(1 − 2GM/(rc²)). Substituting M = 0:

```
   alpha(M=0) = sqrt(1 - 0) = 1                                   (T129.1)
```

**SymPy verification:** residual = 0. [VERIFIED] n_PDTP(M=0) = 1 identically,
for all r > 0.

**Step 3** [Part 74b, Eq 74b.3]: the Sakharov cutoff Λ = m_cond·c/ℏ contains
no M at all — it is a property of the condensate (m_cond), not of any
particular test mass:

```
   d(Lambda)/dM = 0                                                (T129.2)
```

**SymPy verification:** trivial symbolic differentiation, confirmed 0. [VERIFIED]

**Step 4** [Conclusion]: since the calculation fixing N_eff is conducted
entirely in the M = 0 regime (Step 3), and n_PDTP ≡ 1 in that regime (Step 2),
and the dispersion relation it would need to modify contains no α term to
begin with (Step 1) — n_PDTP has no path into this calculation. Confirmed
numerically: inserting a hypothetical correction factor n^p into Λ for
p ∈ {±1, ±2, ±3} evaluates to exactly 1.0 in every case at M = 0. [COMPUTED]

### 10.3 Relation to Part 111 (T7) — Do Not Conflate

Part 111 (T7) found a similarly-flavored negative result — n_PDTP does not
modify the Hawking temperature — but via a **different mechanism**: T_H is
set by *group* velocity (c_group = c always, Part 34), while n_PDTP rescales
only *phase* velocity, and that argument is evaluated *at the horizon*, where
α is emphatically **not** 1 (α(r_S) = 0 exactly). This section's argument is
structurally different: it is not a group-vs-phase-velocity distinction at
all, but a domain-separation argument (the calculation never leaves M = 0,
where n_PDTP is trivial throughout). Both land on "unchanged"; recorded
separately so the two are not mistaken for the same finding.

### 10.4 Two-Phase Check [VERIFIED, Sudoku requirement 4]

Part 61's two-phase Lagrangian splits φ into φ₊ (bulk/gravity) and φ₋
(surface) — same underlying condensate, same m_cond, hence the identical,
still M-independent cutoff Λ. N_eff(two-phase) = 10 (Part 83.4) uses this same
Λ; Sections 10.1-10.3's argument is unaffected by the two-phase extension.

### 10.5 Sudoku Scorecard (T12 / Part 129)

| # | Test | Verdict |
|---|------|---------|
| T1 | α not a free symbol in ω_photon² | PASS |
| T2 | α not a free symbol in ω_vortex² | PASS |
| T3 | α(M=0) = 1 identically (SymPy residual 0) | PASS |
| T4 | dΛ/dM = 0 (cutoff is M-independent) | PASS |
| T5 | N_eff(minimal) = 8 (RECHECK vs Part 83.3) | PASS |
| T6 | N_eff(two-phase) = 10 (RECHECK vs Part 83.4) | PASS |
| T7 | N_eff(+matter) = 34 (RECHECK vs Part 83.5) | PASS |
| T8 | G_ind/G(N_s=8) matches Part 83's 2.356 within 0.1% | PASS |
| T9 | target 6π lies within [N_minimal, N_matter] range | PASS |
| T10 | hypothetical n^p correction = 1.0 at M=0, all tested p | PASS |
| T11 | two-phase cutoff identical to single-phase | PASS |
| T12 | T7 and T12 recorded as independent findings | PASS |

**Score: 12/12 PASS**

### 10.6 What T12 Changes

Nothing about the N_eff gap itself — Part 83's characterization (range
[8, 34], target 6π ≈ 18.85, PARTIAL status) stands exactly as it was. What
changes: the tan/refractive-index investigation (TODO_04) can now cross off
T12 with a clean, well-grounded negative result rather than an open question,
and future readers have an explicit citable reason n_PDTP was not expected to
help here, preventing the question from being re-asked without context.
...
9. Part 95: emergent_c.md — C1 dispersion relations (Results 3/7)
10. Part 98: pdtp_refractive_index.md — n_PDTP = 1/α definition
11. Part 111: hawking_n_pdtp.md — T7's group-vs-phase-velocity negative result
