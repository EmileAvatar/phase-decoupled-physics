...
## Part 41: Non-Perturbative Lattice at Physical Beta -- COMPLETED (2026-03-07)
...
**Key findings:**
- At physical beta, the SC formula sigma_lat = ln(2N/beta) fails completely:
  ln(2*3/6.0) = ln(1) = 0  (zero string tension -- completely wrong).
- Wilson loops W(R,T) are measured directly from MC at beta = 6.0.
- Small-step Metropolis (epsilon=0.10) gives ~85% acceptance at beta=6.0.
- Mean plaquette transitions: <P> = 0.003 (K_NAT) -> 0.316 (beta=6.0). [S21 PASS]
- Area law confirmed qualitatively: W(R+1,T) < W(R,T). [S23 PASS]
- V(R) increases with R (confining). [S24 PASS]
- Lattice spacing from Necco-Sommer (2001): a_lat = 0.093 fm at beta=6.0. [S27 PASS]
- **CPU demo (N=4^4) limitation:** sigma = 4.68 GeV^2 (factor 26 off). [S26 FAIL -- expected]
  Box = 0.37 fm < confinement onset ~0.5 fm; R_max=2 lattice spacings = Coulomb regime.
- **N>=16 required** (GPU/CuPy) for box > 1.5 fm and R up to 6 spacings.
  Expected result at N=16: sigma ~ 0.18 GeV^2 (standard quenched QCD).
- Sudoku score: 6/7 (S26 fails by design -- box too small for confining regime).

**Progression:**
```
Part 38  SU(3) SC 2D         : sigma = 0.173 GeV^2  (4% off; analytic)
Part 39  SU(3) SC 4D         : sigma = 0.173 GeV^2  (4% off; confirmed)
Part 40  + Wilson fermions   : sigma = 0.163 GeV^2  (9% off; quarks worsen)
Part 41  physical beta (4^4) : sigma = 4.68 GeV^2   (box too small, expected)
Part 41  physical beta (N=16): sigma ~ 0.18 GeV^2   (GPU target; Creutz 1980)
```
...
## Part 42: GPU Run at Physical Beta -- NEXT TASK

**Goal:** Run `su3_physical_beta.py` with N=16 on the RTX 3060 GPU (CuPy) to
get a quantitative sigma at physical beta.

**Why:** The CPU 4^4 demo (Part 41) proves the method but cannot resolve the
confining regime (needs R > 0.5 fm = 5+ lattice spacings at beta=6.0).
A 16^4 lattice gives box = 1.5 fm with R up to 8 spacings.

**Tasks:**
1. Install CuPy for RTX 3060 (CUDA 12.x): pip install cupy-cuda12x
2. Add GPU support to su3_physical_beta.py (adapt CuPy arrays)
3. Run: python su3_physical_beta.py --N 16 --beta 6.0 --meas 500
4. Verify Cornell fit gives sigma_lat ~ 0.04 -> sigma_phys ~ 0.18 GeV^2
5. Optionally scan beta = 5.7, 5.9, 6.0, 6.2 to confirm scaling window
